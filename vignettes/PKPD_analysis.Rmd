...
df_full <- apmx::pk_build(ex = ex, pc = pc, pd = pd,
                          ...
                          BDV = F, #calculates baseline dependent variable for PD events
                          DDV = F, #calculates change (delta) from baseline for PD events
                          PDV = F, #calculates percent change from baseline for PD events
                          demo.map = T, #adds specific numeric mapping for SEX, RACE, and ETHNIC variables
                          tv.cov.fill = "downup", #fill pattern for time-varying covariates
                          keep.other = T) #keep or drop all EVID = 2 rows
```
...
df_cov_apply <- apmx::pk_build(ex = ex, pc = pc,
  ...
  apmx::cov_apply(dm) %>%
  apmx::cov_apply(lb) %>%
  apmx::cov_apply(talt, time.by = "DTIM", keep.other = F) %>%
  apmx::cov_apply(tast, time.by = "NTFD", keep.other = F) %>%
  apmx::cov_apply(exposure, id.by = "ID", exp = T) %>%
  apmx::cov_apply(parameters, id.by = "ID", ebe = T)
```
...
```{r}
apmx::pk_build(ex, pc, pd, DDV=T, PDV==T)
```
...
df_warning <- apmx::pk_build(ex, pc, pd_warning, BDV=T)
```
...
```{r}
out.dir <- paste0(getwd(), "/PK_ABC101_V01.csv")
apmx::pk_write(df_combine, out.dir)
```
...
We will start by exploring the definition file feature. The definition file sources variable names from a local list stored on your computer. You can download a simple version with `apmx::variable_list_export()`. Let's start by downloading the list.

```{r}
vl.dir <- paste0(getwd(), "/vl.csv")
apmx::variable_list_export(vl.dir)
```

Open up the variable list and take a look at the information inside. You can customize the descriptions of these variables if you like. You should add definitions for the custom covariates in this project. When you add covariates, do not add the prefix and suffix, just add the root term. For example, you do not need to define `NSEX` and `NSEXC`, just `SEX`.  

Save and close the variable list before continuing.  

An optional component of the definition file is the template (.docx file). It provides a customized look to the definition file.  

Now, let's create the definition file.  

```{r}
apmx::pk_define(file = out.dir,
                project = "ABC999 tutorial",
                variable.list = vl.dir)
```

The file automatically reads out to the same directory as the dataset. Take a look at the definition file and familiarize yourself with the layout. Notice if you forgot to add your custom covariates to the variable list, the covariate is retained in the file but the definition is NA. This document makes it easy to communicate to others what variables are in your dataset.  
...
Similar to the definition function, we can provide a template for formatting. You can also provide a comment to describe the source data. You also have to specify how to group the dataset to compare against the reference dataset. For PK(PD) datasets, you should group by `USUBJID`, `ATFD`, `EVID`, and `DVID`.  

```{r}
apmx::version_log(file = out.dir,
                  orig = T,
                  comp_var = c("USUBJID", "ATFD", "EVID", "DVID"),
                  src_data = "test data")
```

Open the version log document and take a look around. Notice that there is a column called "Comments". You can add a comment there in the word document, and the function will not overwrite it.  

What if we update the dataset and create a new version? Let's add an additional covariate and change another one.  

``` {r}
df_combine2 <- df_combine %>%
  dplyr::group_by(ID) %>%
  dplyr::mutate(BCREAT = round(rnorm(1, 0.85, 0.2), 3), #change BCREAT
                BTUM = rnorm(1, 100, 30)) %>% #add BTUM (baseline tumor size)
  dplyr::ungroup()

out.dir <- paste0(getwd(), "/PK_ABC101_V02.csv")
apmx::pk_write(df_combine2, out.dir)

apmx::version_log(file = out.dir,
                  comp_var = c("USUBJID", "ATFD", "EVID", "DVID"),
                  src_data = "new test data")
```

Now we can communicate to others what is contained in each dataset and why they are changing over the course of the project.  
...
* `ignore.C` will remove all C-flagged records from the analysi.
...
```{r}
apmx::pk_summarize(file = out.dir,
